...
class ShooterScene extends Scene {
    ...
    create() {
        ...
        this.physics.add.collider(this.bullets, this.meteors, (bullet: Bullet, meteor: Meteor) => {
            if (meteor.active && bullet.active) {
                this.points.setText((++ShooterScene.hits).toString())
                meteor.kill();
                bullet.kill();
            }
        }, null, this);
    ...
    static getScores() {
        fetch('https://localhost:5001/api/highscoreLists').then(response => {
            response.json().then(highscores => {
                ShooterScene.generateTable(highscores);
            })
        })
    }

    static generateTable (highscores) {
        ...
        for (const score of highscores.scores) {
            const row = document.createElement("tr");
            ...
            const cell = document.createElement("td");
            cellText = document.createTextNode(score.name);
            cell.appendChild(cellText);
            row.appendChild(cell);

            const cell2 = document.createElement("td");
            cellText = document.createTextNode(score.points);
            cell2.appendChild(cellText);
            row.appendChild(cell2);
